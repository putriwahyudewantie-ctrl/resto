...
<div class="top-banner">
    <div>
        <h2><i class="fas fa-chair me-2"></i>Cek Ketersediaan Meja</h2>
        <p>Tentukan waktu kunjungan Anda untuk melihat meja yang tersedia</p>
    </div>
    @if(auth()->user()->role === 'admin')
    <a href="{{ route('meja.create') }}" class="btn btn-light btn-lg rounded-pill shadow-sm">
...
@if(!empty($tanggal) && !empty($jam))
    <div class="d-flex align-items-center justify-content-between mb-4 mt-4">
        ...
        <span class="badge bg-primary rounded-pill px-3 py-2 shadow-sm">Total: {{ count($mejas) }} Meja Tersedia</span>
    </div>
            ...
            <div class="meja-card @if($isBooked) border-danger @endif" style="{{ $isBooked ? 'opacity: 0.7;' : '' }}">
                ...
                @else
                    ...
                    <a href="{{ url('/booking/create?meja_id=' . $meja->id . '&nomor_meja=' . $meja->no_meja . '&jumlah_orang=' . $meja->kapasitas . '&tanggal_booking=' . $tanggal . '&jam_booking=' . $jam) }}" 
                       class="btn btn-primary w-100 shadow-sm shadow-hover">
                        Booking Sekarang
                    </a>
                @endif
...
<style>
    ...
    .meja-card:hover {
        ...
        box-shadow: 0 20px 50px rgba(15, 47, 102, 0.1);
    ...
    .meja-icon { 
        font-size: 3.5rem; 
        margin-bottom: 20px;
        background: linear-gradient(135deg, #0f2f66 0%, #3b82f6 100%);
    ...
    .meja-no { font-size: 1.7rem; font-weight: 800; color: #0f2f66; margin-bottom: 5px; }
    ...
    .filter-box { 
        background: linear-gradient(135deg, #0f2f66 0%, #1d5bc0 100%); 
        border-radius: 30px; 
        padding: 40px; 
        box-shadow: 0 20px 50px rgba(15,47,102,0.25);
        color: white;
    }
